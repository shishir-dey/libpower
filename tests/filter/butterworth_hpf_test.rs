...
mod tests {
    ...
    use libm::sinf;
    use libpower::filter::butterworth_hpf::*;
    ...
    fn test_order_bounds() {
        let mut filter_low = ButterworthHPF::<4>::new_uninit();
        filter_low.init(1, 1000.0, 44100.0);
        assert_eq!(filter_low.get_order(), 2); // Minimum order is 2

        let mut filter_high = ButterworthHPF::<4>::new_uninit();
    ...
    fn test_passband() {
        ...
        // Test with frequency well above cutoff (2kHz)
        let freq = 2000.0;
        ...
        // Test with high frequency sine wave
        for i in 0..1000 {
            let t = i as f32 / fs;
